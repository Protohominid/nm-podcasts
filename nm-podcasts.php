...
add_action( 'cmb2_admin_init', 'nm_podcast_metaboxes' );
...
function nm_podcast_metaboxes( $meta_boxes ) {
	...
	$cmb = new_cmb2_box( array(
		...
		'object_types'  => array( 'nm_podcasts', ), // Post type
		'context'       => 'normal',
		'priority'      => 'high',
	...
	$cmb->add_field( array(
		'name'       => __( 'File URL', 'cmb2' ),
		'desc'       => __( 'Direct link to file. Should be mp3 format and http protocol (not https).', 'cmb2' ),
		'id'         => $prefix . 'file_url',
		'type'       => 'text_url',
	...
	$cmb->add_field( array(
		'name'       => __( 'File Size', 'cmb2' ),
		'desc'       => __( 'In bytes', 'cmb2' ),
		'id'         => $prefix . 'file_size',
		'type'       => 'text',
		'show_on_cb' => 'cmb2_hide_if_no_cats', // function should return a bool value
	) );
...
// Add the custom feed
//--------------------------------------------------------------------------------------------
add_action( 'init', 'init_nm_podcast_rss' );
function init_nm_podcast_rss(){
...
add_filter( 'feed_content_type', 'nm_podcast_feed_type', 10, 2 );
function nm_podcast_feed_type( $content_type, $type ) {
	if ( 'podcast' === $type ) {
...
function nm_podcasts_filter_content( $content ) {
	if ( !is_admin() ){
		...
		if( is_single() && !empty( $nm_podcast ) && in_the_loop() ) {
			...
			<ul class="podcast" style="text-align: left;">
				<li class="mp3"><a href="' . esc_url( $pod_url ) . '" rel="nofollow">mp3 Download</a></li>
				<li class="itunes"><a href="' . esc_url( $podcasts_options['itunes_subscribe_url'] ) . '" rel="nofollow">Subscribe via iTunes</a></li>';
	
			if( !empty( $podcasts_options['stitcher_subscribe_url'] ) ) {
				$output .= '<li class="stitcher"><a href="' . esc_url( $podcasts_options['stitcher_subscribe_url'] ) . '" rel="nofollow">Subscribe via Stitcher</a></li>';
			}
